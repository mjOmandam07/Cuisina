...
def viewpost(recipe_id):
    ...
    if 'user' in session:
        ...
        if form.validate_on_submit():
            ...
            rate = models.chef(recipe_id=recipe_id, user_id=user[0][0], rating=form_rate.rate.data,
            ...
            if request.form["search"]:
                ...
                flash('Please Search recipe or chef', 'danger')
                return redirect(url_for('viewpost', recipe_id=recipe_id))
        return render_template('view-post.html', active='home', user_request=user_request, user=user,
                               suggested_chef=suggested_chef, recipe=recipe, comments=comments, form=form,
                               form_rate=form_rate, rate=currentRating)
    else:
        return redirect(url_for('login'))
...
def ordersFeed(fltr):
    ...
    if 'user' in session:
        ...
        return render_template('ordersfeed.html', user_request=user_request, active='order', user=user,
                               suggested_chef=suggested_chef, recipe=recipe, form=form, fltr=fltr)
    else:
        return redirect(url_for('login'))
    ...
    if 'user' in session:
        ...
        if form.validate_on_submit() and request.method == 'POST':
            ...
            if request.form["search"]:
                ...
                return redirect(url_for('filter'))
            return redirect(url_for('viewpost', recipe_id=latest))
        return render_template('home.html', active='hot', user_request=user_request, user=user,
                               suggested_chef=suggested_chef, recipe=recipe, form=form, fltr=fltr, profile=profile)
    else:
        return redirect(url_for('login'))
...
def search(search_content, fltr):
    ...
    if 'user' in session:
        ...
        return redirect(url_for('login'))
    return render_template('search.html', search_content=search_content, user=user, recipe=recipe,
                           search_user=search_user, suggested_chef=suggested_chef, fltr=fltr)
